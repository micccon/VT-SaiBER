...
from src.agents.fuzzer.outcome import FuzzerOutcome, WebFinding
...
def test_fuzzer_exposes_run_entrypoint():
    agent = FuzzerAgent(
...
@pytest.mark.asyncio
async def test_fuzzer_returns_validation_error_without_http_target():
    out = await FuzzerAgent(
        execution_runner=_FakeExecutionRunner(
            ExecutionResult(outcome=FuzzerOutcome(base_url="http://example.com"))
        )
    ).run(_base_state())
...
@pytest.mark.asyncio
async def test_fuzzer_persists_structured_findings():
    state = _base_state()
    ...
    out = await FuzzerAgent(execution_runner=runner).run(state)
...
@pytest.mark.asyncio
async def test_fuzzer_uses_normalized_fallback_when_sparse():
    state = _base_state()
    state["discovered_targets"] = {
        "192.168.1.10": {
            "ports": [443],
...
@pytest.mark.asyncio
async def test_fuzzer_rejects_model_output_without_web_tool_use():
    state = _base_state()
    state["discovered_targets"] = {
        "192.168.1.10": {
            "ports": [80],
