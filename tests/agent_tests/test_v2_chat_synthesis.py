...
from src.v2.chat import runner as runner_mod
...
def test_chat_synthesis_runner_parses_structured_output():
    ...
    result = _run(
        runner_mod.V2ChatSynthesisRunner(client=_FakeClient(response)).run(
            _spec(),
            user_input="hello",
        )
    )
...
def test_chat_synthesis_runner_uses_runtime_configured_client_when_not_injected(monkeypatch):
    ...
    result = _run(runner_mod.V2ChatSynthesisRunner().run(_spec(), user_input="hello"))
...
def test_chat_synthesis_runner_supports_ainvoke_clients():
    result = _run(
        runner_mod.V2ChatSynthesisRunner(client=_FakeInvokeClient()).run(
            _spec(),
            user_input="hello",
        )
    )
...
def test_chat_synthesis_runner_raises_for_unsupported_client():
    with pytest.raises(RuntimeError):
        _run(runner_mod.V2ChatSynthesisRunner(client=object()).run(_spec(), user_input="hello"))
