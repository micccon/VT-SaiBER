...
def test_scout_exposes_run_entrypoint():
    agent = ScoutAgent(execution_runner=_FakeExecutionRunner(ExecutionResult(outcome=ScoutOutcome())))
    assert hasattr(agent, "run")


@pytest.mark.asyncio
async def test_scout_returns_validation_error_without_scope():
    state = _base_state()
    state["target_scope"] = []
    out = await ScoutAgent(
...
@pytest.mark.asyncio
async def test_scout_persists_in_scope_discovered_hosts():
    ...
    out = await ScoutAgent(execution_runner=runner).run(_base_state())
    assert sorted(out["discovered_targets"].keys()) == ["192.168.1.20"]
    assert out["agent_log"][0].action == "recon_scan"


@pytest.mark.asyncio
async def test_scout_persists_service_probe_targets():
    runner = _FakeExecutionRunner(
        ExecutionResult(
            outcome=ScoutOutcome(
                targets=[
...
@pytest.mark.asyncio
async def test_scout_rejects_model_output_without_recon_tool_use():
    runner = _FakeExecutionRunner(
        ExecutionResult(
            outcome=ScoutOutcome(
