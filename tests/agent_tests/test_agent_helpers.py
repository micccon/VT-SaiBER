...
from src.agents import common as common_mod
...
def test_run_agent_node_persists_updates(monkeypatch):
    persisted: list[dict[str, Any]] = []

    class _FakeAgent:
        async def run(self, state):
            return {"current_agent": "demo", "iteration_count": 3}
    ...
    out = _run(common_mod.run_agent_node(_base_state(), _FakeAgent))

    assert out["current_agent"] == "demo"
    assert persisted == [{"current_agent": "demo", "iteration_count": 3}]
