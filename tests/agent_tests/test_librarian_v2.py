...
from src.v2.agents.librarian.context import build_research_query
...
def _enable_llm(agent: LibrarianV2Agent) -> LibrarianV2Agent:
...
def test_librarian_v2_research_query_matches_legacy_librarian():
    state = _base_state()
    ...
    assert build_research_query(state) == LibrarianAgent()._build_research_query(state)


def test_librarian_v2_exposes_run_entrypoint():
    agent = LibrarianV2Agent(
...
@pytest.mark.asyncio
async def test_librarian_v2_cache_hit_reuses_brief_without_synthesis():
    state = _base_state()
    query = build_research_query(state)
    cache_key = LibrarianAgent()._cache_key(query)
...
@pytest.mark.asyncio
async def test_librarian_v2_cache_miss_runs_synthesis_and_writes_cache():
    state = _base_state()
    runner = _FakeSynthesisRunner(outcome=_brief(summary="Synthesized summary"))
    agent = _enable_llm(LibrarianV2Agent(
        rag_orchestrator=_FakeRag(),
        synthesis_runner=runner,
    ))
...
@pytest.mark.asyncio
async def test_librarian_v2_retrieval_metadata_overrides_model_claims():
    state = _base_state()
    kb_results = [
...
@pytest.mark.asyncio
async def test_librarian_v2_falls_back_when_synthesis_fails():
    state = _base_state()
    kb_results = [
