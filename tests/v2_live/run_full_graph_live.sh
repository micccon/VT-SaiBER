#!/bin/bash
# Run the live full supervisor-led v2 graph smoke test inside the agents container.
...
docker exec -t \
    ...
    -e V2_TRACE_MAX_CHARS="${V2_TRACE_MAX_CHARS:-4000}" \
    ...
    "cd /app && python3 -m pytest tests/v2_live/test_full_graph_live.py -q -s --log-cli-level=INFO --log-cli-format='%(asctime)s [%(levelname)s] %(name)s: %(message)s'"
