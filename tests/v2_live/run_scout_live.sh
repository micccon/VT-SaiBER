#!/bin/bash
# Run the live scout_v2 test inside the agents container.
...
docker exec -t \
    ...
    "cd /app && python3 -m pytest tests/v2_live/test_scout_live.py -q --log-cli-level=INFO --log-cli-format='%(asctime)s [%(levelname)s] %(name)s: %(message)s'"
