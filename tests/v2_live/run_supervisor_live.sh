#!/bin/bash
# Run the live supervisor_v2 test inside the agents container.
...
docker exec -t \
    ...
    "cd /app && python3 -m pytest tests/v2_live/test_supervisor_live.py -q --log-cli-level=INFO --log-cli-format='%(asctime)s [%(levelname)s] %(name)s: %(message)s'"
