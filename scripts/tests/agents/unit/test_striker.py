...
from src.agents.striker.outcome import ArtifactClaim, SessionClaim, StrikerOutcome
from src.agents.striker.policy import StrikerExecutionPolicy
from src.runtime.contracts import ApprovalEvent, ExecutionResult, ToolEvent, ToolSpec
...
def test_striker_exposes_run_entrypoint():
...
def test_striker_marks_only_high_impact_tools_approval_required():
    spec = striker_mod.StrikerAgent(
        execution_runner=_FakeExecutionRunner(
            ExecutionResult(outcome=StrikerOutcome(status="no_candidate"))
        )
...
@pytest.mark.asyncio
async def test_striker_returns_validation_error_without_targets():
    agent = striker_mod.StrikerAgent(
        execution_runner=_FakeExecutionRunner(
            ExecutionResult(outcome=StrikerOutcome(status="no_candidate"))
        )
...
@pytest.mark.asyncio
async def test_striker_preserves_model_status_by_default():
    ...
    result = await striker_mod.StrikerAgent(execution_runner=runner).run(_build_state())
...
@pytest.mark.asyncio
async def test_striker_approval_blocked_overrides_model_claim():
    runner = _FakeExecutionRunner(
        ExecutionResult(
            outcome=StrikerOutcome(
...
@pytest.mark.asyncio
async def test_striker_session_opened_requires_verified_session_evidence():
    runner = _FakeExecutionRunner(
        ExecutionResult(
            outcome=StrikerOutcome(
                status="session_opened",
...
@pytest.mark.asyncio
async def test_striker_verified_session_updates_machine_state():
    runner = _FakeExecutionRunner(
        ExecutionResult(
            outcome=StrikerOutcome(
                status="session_opened",
...
@pytest.mark.asyncio
async def test_striker_only_persists_verified_artifacts():
    runner = _FakeExecutionRunner(
        ExecutionResult(
            outcome=StrikerOutcome(
...
@pytest.mark.asyncio
async def test_striker_writes_compact_machine_state_for_supervisor_and_resident():
    runner = _FakeExecutionRunner(
        ExecutionResult(
            outcome=StrikerOutcome(
                status="validated_no_session",
...
def test_striker_policy_only_gates_approval_required_tools(monkeypatch):
    ...
    monkeypatch.setattr("src.agents.striker.policy.require_manual_approval", fake_require_manual_approval)
