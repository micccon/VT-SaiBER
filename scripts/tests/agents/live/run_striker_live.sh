#!/bin/bash
# Run the live striker manual-approval exploit-attempt test inside the agents container.
...
docker exec -it \
    ...
    "cd /app && python3 -m pytest scripts/tests/agents/live/test_striker_live.py -q -s --log-cli-level=INFO --log-cli-format='%(asctime)s [%(levelname)s] %(name)s: %(message)s'"
