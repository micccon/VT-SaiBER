#!/bin/bash
# Run the live supervisor test inside the agents container.
...
docker exec -t \
    ...
    "cd /app && python3 -m pytest scripts/tests/agents/live/test_supervisor_live.py -q --log-cli-level=INFO --log-cli-format='%(asctime)s [%(levelname)s] %(name)s: %(message)s'"
