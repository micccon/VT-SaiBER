...
from src.agents.fuzzer.constants import FUZZER_ALLOWED_TOOLS
from src.agents.resident.constants import RESIDENT_ALLOWED_TOOLS
from src.agents.scout.constants import ATTACKBOX_MCP_URL, SCOUT_ALLOWED_TOOLS
from src.agents.striker.constants import STRIKER_ALLOWED_TOOLS
from src.runtime.execution import _normalize_mcp_call_result
from scripts.tests.helpers import CAPTURED_BASE_URL, CAPTURED_TARGET, step
...
@pytest.mark.asyncio
async def test_attackbox_mcp_advertises_all_tools():
    """Verify every-required MCP tool is advertised by attackbox."""
    ...
    step(f"expected tools present: {sorted(EXPECTED_ATTACKBOX_TOOLS - set(missing))}")
    if missing:
        pytest.fail(f"Attackbox MCP missing-required tools: {missing}")
