#!/bin/bash
# Run the live full supervisor-led production graph smoke test inside the agents container.
...
docker exec -it \
    ...
    -e SAIBER_TRACE_MAX_CHARS="${SAIBER_TRACE_MAX_CHARS:-4000}" \
    ...
    "cd /app && python3 -m pytest scripts/tests/infrastructure/live/test_full_graph_live.py -q -s --log-cli-level=INFO --log-cli-format='%(asctime)s [%(levelname)s] %(name)s: %(message)s'"
