"""Context-building helpers for supervisor v2."""
...
def latest_resident_outcome(state: CyberState) -> dict[str, Any]:
    ...
    for entry in reversed(state.get("agent_log", []) or []):
        ...
        if agent_name not in {"resident", "resident_v2"}:
            continue
...
def has_librarian_research(state: CyberState) -> bool:
    ...
    return bool(
        ...
        or has_agent_run(agent_log, "librarian_v2")
...
def striker_failed_recently(state: CyberState) -> bool:
    ...
    recent_agents = [
        ...
        if name not in {"supervisor", "supervisor_v2"}
    ]
    return bool(recent_agents and recent_agents[-1] in {"striker", "striker_v2"} and not (state.get("active_sessions", {}) or {}))
