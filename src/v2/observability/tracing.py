"""Production tracing for v2 agent runs.

Tracing is opt-in and intentionally log-based so Docker deployments can collect
it without a separate dependency. Raw model/tool payloads stay redacted unless
explicitly enabled through runtime config.
"""
...
LOGGER = logging.getLogger("src.v2.trace")
SENSITIVE_KEY_PARTS = ("api_key", "apikey", "authorization", "bearer", "password", "secret", "token")


@dataclass(frozen=True)
class V2TraceSettings:
    """Runtime tracing controls for v2 lanes."""
...
def get_v2_trace_settings() -> V2TraceSettings:
    ...
    try:
        ...
        return V2TraceSettings(
            enabled=bool(getattr(config, "v2_trace_enabled", False)),
            include_raw=bool(getattr(config, "v2_trace_include_raw", False)),
            max_chars=max(200, int(getattr(config, "v2_trace_max_chars", 2000) or 2000)),
        )
    except Exception:
        return V2TraceSettings()
...
def trace_execution_start(
    *,
    ...
    """Log the start of a v2 tool-execution run."""

    settings = get_v2_trace_settings()
...
def trace_execution_result(*, agent_name: str, result: ExecutionResult[Any]) -> None:
    """Log a compact summary of a v2 tool-execution result."""
...
def trace_synthesis_start(*, agent_name: str, model: str, history_count: int) -> None:
    """Log the start of a v2 chat/synthesis run."""
...
def trace_synthesis_result(*, agent_name: str, result: ChatSynthesisResult[Any]) -> None:
    """Log a compact summary of a v2 chat/synthesis result."""
...
def trace_failure(*, lane: str, agent_name: str, exc: Exception) -> None:
    """Log a v2 runner failure without dumping raw request state."""
...
def _emit(event: str, payload: dict[str, Any], *, settings: V2TraceSettings) -> None:
    envelope = {"event": event, **payload}
    LOGGER.info("v2_trace %s", _preview(envelope, max_chars=settings.max_chars))


def _maybe_payload(value: Any, *, settings: V2TraceSettings) -> Any:
