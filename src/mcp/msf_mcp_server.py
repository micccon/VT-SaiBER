...
def _normalize_module_type(module_type: str) -> str:
    ...
    if normalized not in VALID_MODULE_TYPES:
        raise ValueError(
            f"Unsupported module_type '{module_type}'. "
            f"Use one of: {sorted(VALID_MODULE_TYPES)}"
        )
    return normalized
...
@mcp.tool(name="get_module_options")
async def get_module_options(
    ...
    """
    Return module option metadata, similar to msfconsole 'show options'
    and (if advanced=true) 'show advanced'.

    Args:
        module_type: Module type (exploit, auxiliary, post, payload, encoder, nop).
        module_name: Module name/path (e.g., scanner/ssh/ssh_login).
        search: Optional case-insensitive filter on option name/description.
        advanced: If true, include advanced/evasion options.

    Returns:
        Dictionary with option metadata and counts.
    """
    try:
        ...
        for name in sorted({str(v) for v in option_names}):
            info = option_info.get(name, {})
            if not isinstance(info, dict):
                info = {}

            is_advanced = bool(info.get("advanced", False) or info.get("evasion", False))
            if not advanced and is_advanced:
                continue
            ...
            if search_term:
                ...
                if search_term not in name_l and search_term not in desc_l:
                    continue

            required = bool(info.get("required", name in required_set))
            entry: Dict[str, Any] = {
                "name": name,
                "required": required,
            ...
            if "default" in info:
                entry["default"] = info.get("default")
            if "enums" in info:
                entry["enums"] = info.get("enums")
            if "evasion" in info:
                entry["evasion"] = bool(info.get("evasion"))
        ...
        return {
            ...
            "search": search,
            "advanced": advanced,
            "count": len(options_out),
            "required_count": sum(1 for option in options_out if option.get("required")),
            "options": options_out,
            "missing_required": missing_required,
        }
    except (ValueError, MsfRpcError) as e:
        logger.error(f"Error retrieving module options for {module_type}/{module_name}: {e}")
        return {"status": "error", "message": str(e)}
    except Exception as e:
        logger.exception(f"Unexpected error retrieving module options for {module_type}/{module_name}")
        return {"status": "error", "message": f"Unexpected error retrieving module options: {e}"}


@mcp.tool(name="get_module_info")
async def get_module_info(module_type: str, module_name: str) -> Dict[str, Any]:
    """
    Return concise module metadata, similar to msfconsole 'info <module>'.
    ...
    Returns:
        Module summary including description, references, targets, and payload hints.
    """
    try:
        normalized_type = _normalize_module_type(module_type)
        module_obj = await _get_module_object(normalized_type, module_name)
        ...
        for name in sorted(set(option_names)):
            ...
            if not isinstance(info, dict):
                info = {}
            if bool(info.get("required", name in required_set)):
        ...
        return {
            ...
            "module_type": normalized_type,
            "module": full_module_path,
            "name": str(getattr(module_obj, "name", module_name) or module_name),
            "rank": rank,
            ...
            "required_count": len(required_options),
            "compatible_payloads": compatible_payloads,
        }
    except (ValueError, MsfRpcError) as e:
        logger.error(f"Error retrieving module info for {module_type}/{module_name}: {e}")
        return {"status": "error", "message": str(e)}
    except Exception as e:
        logger.exception(f"Unexpected error retrieving module info for {module_type}/{module_name}")
        return {"status": "error", "message": f"Unexpected error retrieving module info: {e}"}

@mcp.tool(name="generate_payload")
async def generate_payload(
    ...
    Args:
        ...
        nop_sled_size: Optional NOP sled size.
...
@mcp.tool(name="run_auxiliary_module")
...
@mcp.tool(name="send_session_command")
...
@mcp.tool(name="terminate_session")
